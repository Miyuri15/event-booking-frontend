...
export function fetchPublicEvents() {
  return request(API_BASE_URL, "/api/events/public", {});
}
...
export function bookEvent(eventId, payload, token) {
  return request(API_BASE_URL, `/api/events/${eventId}/book`, {
    method: "POST",
    body: payload,
    token,
  });
...
export function createStripeCheckoutSession(items, token) {
